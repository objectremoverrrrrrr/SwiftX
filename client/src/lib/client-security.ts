...
function initializeClickjackingProtection(): void {
  // Only check for clickjacking in production environments
  // Allow iframe embedding in development/replit environment
  const isProduction = window.location.protocol === 'https:' && !window.location.hostname.includes('replit');
  
  if (isProduction && window !== window.top) {
