...
export const jwtPattern: DetectionPattern = {
  ...
  validator: (text: string) => {
    ...
    return parts.length === 3 && parts.every(part => /^[A-Za-z0-9_-]+$/.test(part) || part === '');
  },
  decoder: (text: string) => {
    try {
      const parts = text.split('.');
      ...
      // Decode Base64URL (replace - with +, _ with /, and add padding)
      ...
      return JSON.stringify({
        ...
        signature: parts[2] ? '[Signature Present]' : '[No Signature]'
      }, null, 2);
    } catch {
      return null;
    }
